...
class Cola{
	...
	public:
	Cola(const char *cad=nullptr) {  //funcion en linea
		if(cad){   //cadena!=nullptr
...
\subsection{Punteros Inteligentes (Smart Pointers)}

En C++ moderno (a partir de C++11), se recomienda el uso de \textbf{punteros inteligentes} en lugar de la gestión manual de memoria con \textit{new} y \textit{delete}. Los punteros inteligentes implementan el principio RAII (\textit{Resource Acquisition Is Initialization}), liberando automáticamente la memoria cuando el objeto sale de su ámbito, evitando fugas de memoria y simplificando el código.

Los punteros inteligentes están definidos en la biblioteca \textit{<memory>} y son:

\begin{itemize}
\item \textbf{std::unique\_ptr} - Posesión exclusiva de un recurso. No puede ser copiado, solo movido.
\item \textbf{std::shared\_ptr} - Posesión compartida con conteo de referencias. Múltiples punteros pueden apuntar al mismo objeto.
\item \textbf{std::weak\_ptr} - Referencia no propietaria que no incrementa el contador de referencias. Usado para romper ciclos de referencias.
\end{itemize}

\textcolor{blue}{Ejemplo} con \textit{std::unique\_ptr}:
...
int main(){
	...
	// Uso de unique_ptr - gestion automatica de memoria
	unique_ptr<Cola> pCola = make_unique<Cola>("Cola pCola");
	
	a.set(1);
	b.set(2);
	pCola->set(3);  // acceso con ->
	a.set(11);
	b.set(22);
	pCola->set(33);
	...
	// No es necesario delete - el destructor se llama automaticamente
	// cuando pCola sale de ambito
}
\end{minted}
\caption{Ejemplo de uso de std::unique\_ptr para gestión automática de memoria en C++.}
% \caption[Long Code Example]{A long code example which will break across pages.}
\label{listing:smart_ptr}
\end{longlisting}

\textcolor{blue}{Ejemplo} con \textit{std::shared\_ptr}:
...
fontsize=\footnotesize]
    {cpp}
// Ejemplo de std::shared_ptr - posesion compartida
#include <iostream>
#include <memory>
...
class Recurso {
	...
	Recurso(int v) : valor(v) {
		cout << "Recurso creado: " << valor << endl;
	}
	~Recurso() {
		cout << "Recurso destruido: " << valor << endl;
	}
	void mostrar() {
...
int main() {
	// Crear shared_ptr
	shared_ptr<Recurso> ptr1 = make_shared<Recurso>(42);
	
	cout << "Contador de referencias: " << ptr1.use_count() << endl;
	
	{
		// Compartir posesion
		shared_ptr<Recurso> ptr2 = ptr1;
		cout << "Contador de referencias: " << ptr1.use_count() << endl;
		ptr2->mostrar();
		
		// ptr2 sale de ambito aqui
	}
	
	cout << "Contador de referencias: " << ptr1.use_count() << endl;
	
	// El recurso se destruye automaticamente cuando el último
	// shared_ptr sale de ambito
	return 0;
}
\end{minted}
\caption{Ejemplo de std::shared\_ptr mostrando el conteo de referencias en C++.}
% \caption[Long Code Example]{A long code example which will break across pages.}
\label{listing:shared_ptr}
\end{longlisting}
...
\rowcolor{gray!50} 
\begin{center}
\textbf{Ventajas de los Punteros Inteligentes} 
\end{center} 
\\ \hline
\begin{itemize}
\item Gestión automática de memoria - no es necesario llamar a \textit{delete}.
\item Prevención de fugas de memoria (\textit{memory leaks}).
\item Prevención de doble liberación (\textit{double delete}).
\item Código más seguro y expresivo.
\item Compatible con excepciones - la memoria se libera incluso si ocurre una excepción.
\end{itemize}
\\ \hline
\end{tabular}
\end{center}
