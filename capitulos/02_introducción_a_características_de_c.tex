...
\mint[frame=none,linenos=false]{c}|/*  comentario en C     */|

	En C++ los comentarios pueden ser además de una sola línea:
\mint[frame=none,linenos=false]{cpp}|// este es un comentario en C++|
...
\mint[frame=none,linenos=false]{c}|printf("Número: ");|
\mint[frame=none,linenos=false]{c}|scanf("\%d", &num);|
\mint[frame=none,linenos=false]{c}|printf("El valor leído es: " \%d\n", num);|

Sería en C++ de la siguiente manera:

\mint[frame=none,linenos=false]{cpp}|cout << "Número";|
\mint[frame=none,linenos=false]{cpp}|cin >> num;|
\mint[frame=none,linenos=false]{cpp}|cout << "El valor leído es: " << num << '\n';|
...
\hline
 \mint[frame=none]{cpp}|inline <declaración de la función>|\\
\hline
\end{tabular}
\end{center}
...
\mint[frame=none,linenos=false]{cpp}|punto( , 8);|
...
\mint[frame=none,linenos=false]{cpp}|int *max= new int (1000);|
...
\rowcolor{gray!50}\textbf{Sintaxis}\\ 
\hline
 \mint[frame=none,linenos=false]{cpp}|enum <nombreEnum> { <elem 1>, <elem 2>, …, <elem n> }; |\\
\hline
\end{tabular}
\end{center}
...
Por lo que para el código anterior, la enumeración sería:
 ...
 \mint[frame=none,linenos=false]{cpp}|enum Temporada { PRIMAVERA, VERANO, OTOÑO, INVIERNO } |
...
\rowcolor{gray!50}\textbf{Sintaxis}\\ 
\hline
 \mint[frame=none,linenos=false]{cpp}{| auto <variable>= <valor>;}
 |\\
\hline
\end{tabular}
\end{center}
