...
Por otra parte, las \textit{Aplicaciones Gráficas}, que son programas que utilizan bibliotecas como JavaFX o Swing para crear interfaces de usuario ricas. Anteriormente se utilizaban \textit{Applets}, pero han sido depreciados y eliminados en versiones modernas del JDK.

En términos del código fuente las diferencias entre una aplicación de consola y una gráfica son:

\begin{itemize}
\item  Una aplicación de consola debe definir una clase que contenga el método \textit{main()}, que controla su ejecución.
\item   Una aplicación gráfica con JavaFX hereda de la clase \textit{Application} y sobrescribe el método \textit{start()}.
\end{itemize}
...
\item \textbf{javadoc}. Generador de documentación.
\end{itemize}
...
\hline
 \mint[frame=none,linenos=false]{bash}|javac [opciones] <archivo1.java>|\\
\hline
\end{tabular}
\end{center}
...
%Para la ejecución de una aplicación gráfica se utiliza el mismo comando java, asegurándose de tener las bibliotecas necesarias en el classpath o modulepath.
...
\mint[frame=none,linenos=false]{java}|int x;|
...
\mint[frame=none,linenos=false]{java}|int x = 0;|
...
\mint[frame=none,linenos=false]{java}|op1 = Expresión;|
...
Ejemplo:
\mint[frame=none,linenos=false]{java}|boolean activado = false;|
...
\mint[frame=none,linenos=false]{java}|default: instruccionesPorDefecto;|
...
\mint[frame=none,linenos=false]{java}|tipo nombreDelArreglo[  ];|
...
\mint[frame=none,linenos=false]{java}|new tipoElemento[ <numElementos> ];|
...
\mint[frame=none,linenos=false]{java}|int vector[] = new int[5];|
...
\mint[frame=none,linenos=false]{java}|j = vector[0]; vector[4] = matriz[2][3];|
...
\rowcolor{gray!50}\textbf{Sintaxis}\\ 
\hline
 \mint[frame=none,linenos=false]{java}|enum <nombreEnum> { <elem 1>, <elem 2>, …, <elem n> }|\\
\hline
\end{tabular}
\end{center}
...
\mint[frame=none,linenos=false]{java}|enum Temporada { PRIMAVERA, VERANO, OTOÑO, INVIERNO }|
...
\rowcolor{gray!50}\textbf{Sintaxis}\\ 
\hline
 \mint[frame=none,linenos=false]{java}|package [<ruta>]<nombre paquete>|\\
\hline
\end{tabular}
\end{center}
