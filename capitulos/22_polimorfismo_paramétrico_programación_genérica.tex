...
\subsection{Autoboxing y Unboxing}

Aunque los tipos genéricos en Java no aceptan tipos primitivos directamente, Java proporciona un mecanismo llamado \textbf{autoboxing} (auto-empaquetado) que convierte automáticamente entre tipos primitivos y sus correspondientes clases wrapper (envolventes). Cada tipo primitivo tiene una clase wrapper correspondiente:

\begin{center}
\begin{tabular}{|c|c|}
\hline
\rowcolor{gray!50}
\textbf{Tipo Primitivo} & \textbf{Clase Wrapper} \\
\hline
int & Integer \\
\hline
double & Double \\
...
El \textbf{autoboxing} es la conversión automática que hace el compilador de Java de un tipo primitivo a su clase wrapper correspondiente. El \textbf{unboxing} es la conversión inversa, de la clase wrapper al tipo primitivo.
...
// Autoboxing: el int 5 se convierte automáticamente a Integer
numeros.add(5);  // equivalente a: numeros.add(Integer.valueOf(5));

// Unboxing: el Integer se convierte automáticamente a int
int valor = numeros.get(0);  // equivalente a: int valor = numeros.get(0).intValue();
\end{minted}
...
\end{center} 
\\ \hline
Gracias al autoboxing, es posible usar tipos primitivos con colecciones genéricas de manera transparente. Sin embargo, es importante recordar que internamente se están usando objetos wrapper, lo cual puede tener implicaciones de rendimiento en código crítico debido a la creación de objetos adicionales.
\\ \hline
\end{tabular}
\end{center}
